...
def cuda_dummy_forward(batch: int = 4, seq_len: int = 1024, dim: int = 768, planes: int = 128) -> None:
    ...
    torch.cuda.reset_peak_memory_stats(device); start_time = time.time()
    ...
    with torch.inference_mode():
        out = layer(x)
    torch.cuda.synchronize(device); duration = (time.time() - start_time) * 1000

    peak_mb = torch.cuda.max_memory_allocated(device) / (1024 ** 2)
    print(
        f"[check] cuda dummy forward OK, input=({batch}, {seq_len}, {dim}), "
        f"planes={planes}, output={tuple(out.shape)}, time_ms={duration:.2f}, peak_vram_mb={peak_mb:.2f}, "
...
def cuda_stress_2048() -> None:
    ...
    peak_mb = torch.cuda.max_memory_allocated(device) / (1024 ** 2)
    print(
        f"[check] cuda 2048 stress OK, output={tuple(out.shape)}, time_ms={duration:.2f}, peak_vram_mb={peak_mb:.2f}"
    )
